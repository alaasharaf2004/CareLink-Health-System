...
function RadiologyHomePage() {
  ...
  const reload = () => {
    ...
    setOrders(
      careSystemStore.listImagingOrders().map((order) => ({
        ...order,
  ...
  const submitResults = (event) => {
    ...
    if (editing.patientId) {
      careSystemStore.notifyPatient({
        ...
        body: `تم إكمال تصوير: ${editing.studies || "طلب أشعة"}. يمكنك مراجعتها من تفاصيل الموعد.`,
        type: "radiology",
      });
    }
    showToast("تم إرسال نتيجة الأشعة للطبيب", "success");
    setEditing(null);
    setResultText("");
    setPdfName("");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <FadeUp index={0}>
        <AdminPageHeader
          title="لوحة الأشعة"
          description="استقبل طلبات التصوير من الأطباء، أدخل التقارير، وأرسلها لإكمال مسار الزيارة."
        />
      </FadeUp>
        ...
        <div className="flex flex-wrap gap-2">
          {FILTERS.map((item) => (
            <button
              ...
              className={`workspace-btn-press rounded-full px-4 py-2 text-sm font-bold transition ${
                filter === item.value
      ...
      {filteredOrders.length === 0 ? (
        <EmptyState
          ...
          description="عندما يطلب الطبيب تصويراً شعاعياً، ستظهر هنا لإدخال التقرير."
        />
      ) : (
        <FadeUp index={2}>
          <AdminTable
              ...
              <AdminTableRow key={order.id} className="workspace-list-row">
                ...
                <AdminTableCell>
                  {order.status !== "completed" ? (
                    <button
                      type="button"
                      className="workspace-btn-press inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-blue-700"
                      onClick={() => {
                        setEditing(order);
      ...
      {editing && (
        <Modal title="إدخال تقرير الأشعة" onClose={() => setEditing(null)} maxWidth="max-w-xl">
          <form className="space-y-4" onSubmit={submitResults} dir="rtl">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
              ...
              <p className="mt-1">
                <span className="font-bold text-slate-500">الفحص: </span>
                {editing.studies}
              </p>
            </div>
            ...
            <textarea
              ...
              placeholder="اكتب تقرير التصوير الطبي..."
              value={resultText}
            ...
            <button
              type="submit"
              className="workspace-btn-press inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-bold text-white hover:bg-blue-700"
            >
              <Send size={16} />
              حفظ وإرسال للطبيب
            </button>
...
export default RadiologyHomePage;
